...
describe('quoteBuyExactRounds', function () {
  let admin: Signer;
  let user1: Signer;
  let user2: Signer;
  let user3: Signer;
  ...
  it('CORE_010101', async function (): Promise<void> {
    ...
    await runROMethod({
      method: aMMFacet
        .connect(user1)
        .functions.quoteBuyExactRounds(metaNftId, afterRoundFee(tenPercent)),
      expectedArgs: {
        _baseIn: beforeBaseFee(f10),
      },
    });
  });

  // Quote when some rounds were minted
  it('CORE_010102', async function (): Promise<void> {
    const tags: string[] = [];
    ...
    await runROMethod({
      method: aMMFacet
        ...
        .functions.quoteBuyExactRounds(metaNftId, afterRoundFee(tenPercent)),
      expectedArgs: {
        _baseIn: beforeBaseFee(df10To20),
      },
    });
  });

  // Quote with non-minted _metaNftId
  it('CORE_010103', async function (): Promise<void> {
    const metaNftId = 0;

    await runROMethod({
      method: aMMFacet
        .connect(user1)
        .functions.quoteBuyExactRounds(metaNftId, 10),
      expectRevert: true,
    });
  });

  // Quote with non-latest _metaNftId
  it('CORE_010104', async function (): Promise<void> {
    const tags: string[] = [];
    ...
    await runROMethod({
      method: aMMFacet
        .connect(user1)
        .functions.quoteBuyExactRounds(metaNftId, tenPercent),
      expectRevert: true,
    });
  });

  // Quote when all versions are delisted
  it('CORE_010105', async function (): Promise<void> {
    const tags: string[] = [];
  ...
  // Quote with zero _roundOut
  it('CORE_010106', async function (): Promise<void> {
    const tags: string[] = [];
    ...
    await runROMethod({
      method: aMMFacet
        .connect(user1)
        .functions.quoteBuyExactRounds(metaNftId, zeroEther),
      expectRevert: true,
    });
  });

  // Quote with _roundOut less than round unit
  it('CORE_010107', async function (): Promise<void> {
    const tags: string[] = [];
    ...
    await runROMethod({
      method: aMMFacet
        .connect(user1)
        .functions.quoteBuyExactRounds(metaNftId, roundUnit.sub(1)),
      expectRevert: true,
    });
  });

  // TODO CORE_010108 Quote with min _roundOut that has positive _baseIn

  // TODO CORE_010109 Quote with max _roundOut
});
