...
describe('updateDescriptionHash', function () {
  let admin: Signer;
  let user1: Signer;
  let user2: Signer;
  let user3: Signer;
  ...
  // Update descriptionHash
  it('CORE_021601', async function (): Promise<void> {
    ...
    const descriptionHash = '0x1234123412341234123412341234123412341234123412341234123412341234';
    await listingFacet.connect(user1).updateDescriptionHash(metaNftId, descriptionHash);
    const pairInfo = await viewFacet.getPairInfo(metaNftId);
    expect(pairInfo._descriptionHash).to.equal(descriptionHash);
  });

  // Call by non-pair-owner
  it('CORE_021602', async function (): Promise<void> {
    const listReulst = await runRWMethod({
      method: listingFacet.connect(user1).list(
        testERC721.address,
        tokenId,
    ...
    const metaNftId: BigNumberish = listReulst._metaNftId;
    const descriptionHash = '0x1234123412341234123412341234123412341234123412341234123412341234';
    await expect(listingFacet.updateDescriptionHash(metaNftId, descriptionHash)).to.be.revertedWith('Pilgrim: Must be pair owner');
  });
});
